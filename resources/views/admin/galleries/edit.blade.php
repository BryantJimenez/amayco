						...
						<form action="{{ route('galeria.update', ['slug' => $gallery->slug]) }}" method="POST" class="form" id="formGalleryEdit" enctype="multipart/form-data">
							...
							<div class="row">
								<div class="form-group col-12">
									...
									<input class="form-control @error('title') is-invalid @enderror" type="text" name="title" required placeholder="Introduzca un título" value="{{ $gallery->title }}">
								</div>

								<div class="form-group col-12">
									<label class="col-form-label">Imagen<b class="text-danger">*</b></label>
									<input type="file" name="image" accept="image/*" class="dropify" data-height="125" data-max-file-size="20M" data-allowed-file-extensions="jpg png jpeg web3" data-default-file="{{ image_exist('/admins/img/galleries/', $gallery->image) }}" />
								</div>

								<div class="form-group col-12">
									<label class="col-form-label">Descripción<b class="text-danger">*</b></label>
									<textarea class="form-control @error('description') is-invalid @enderror" required name="description" placeholder="Introduce una descripción" rows="5">{{ $gallery->description }}</textarea>
								</div>
								...
								<div class="form-group col-xl-6 col-lg-6 col-md-6 col-12">
									...
									<select class="form-control @error('category_id') is-invalid @enderror" name="category_id" required>
										...
										<option @if($gallery->category_id==$category->id) selected @endif value="{{ $category->slug }}">{{ $category->name }}</option>
										@endforeach
									</select>
								</div>
								...
								<div class="form-group col-xl-6 col-lg-6 col-md-6 col-12">
									...
									<select class="form-control @error('language_id') is-invalid @enderror" name="language_id" required>
										...
										<option @if($gallery->language_id==$language->id) selected @endif value="{{ $language->slug }}" >{{ $language->name }}</option>
										@endforeach
									</select>
								</div>
